              ...
              <Popover onOpenChange={(open) => {
                ...
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="icon" className="relative h-9 w-9">
                    ...
                    {push.isSubscribed && unreadEvents === 0 && (
                      <span className="absolute top-1.5 right-1.5 w-[8px] h-[8px] bg-emerald-500 rounded-full ring-2 ring-background" />
                    )}
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-80 max-w-[calc(100vw-2rem)] p-0 overflow-hidden" align="end" sideOffset={8}>
                  {/* Push notification toggle bar */}
                  {push.isConfigured && push.permission !== 'unsupported' && (
                      ...
                      <div className="flex items-center justify-between gap-2">
                          ...
                          <div className="min-w-0">
                            ...
                            {push.permission === 'denied' && (
                              <p className="text-[10px] text-muted-foreground truncate">
                                Buka Settings → Notifikasi → Izinkan
                              </p>
                            )}
                          </div>
                        </div>
                        {push.permission !== 'denied' && (
                          <Button
                            ...
                            disabled={push.permission === 'loading'}
                          >
                            {push.permission === 'loading' ? (
                          ...
                          fetch('/api/push/test', {
                            ...
                            .then(data => {
                              if (data.success) {
                                ...
                                toast.error(data.message || 'Gagal mengirim test push');
                              }
                            })
                            .catch(() => toast.error('Gagal mengirim test push'))

...
export async function POST(
  ...
  try {
    ...
    const fundSource = data.fundSource || salary.fund_source;
    ...
    // Step 1: Check pool balance sufficiency
    const poolKey = fundSource === 'hpp_paid' ? 'pool_hpp_paid_balance' : 'pool_profit_paid_balance';
    const { data: poolSetting } = await db.from('settings').select('key, value').eq('key', poolKey).maybeSingle();
    const poolBalance = poolSetting ? (parseFloat(JSON.parse(poolSetting.value)) || 0) : 0;

    if (poolBalance < salary.total_amount) {
      const poolLabel = fundSource === 'hpp_paid' ? 'HPP Sudah Terbayar' : 'Profit Sudah Terbayar';
      throw new Error(`Saldo pool ${poolLabel} tidak mencukupi. Tersedia: ${new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 }).format(poolBalance)}, dibutuhkan: ${new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 }).format(salary.total_amount)}`);
    }
    ...
    try {
      await atomicUpdatePoolBalance(poolKey, -salary.total_amount);
        ...
        throw new Error('Gagal mengurangi saldo pool: ' + (poolErr instanceof Error ? poolErr.message : String(poolErr)));
    }
    ...
    try {
      ...
      // BUG FIX #1: Compensate pool deduction if physical account deduction fails
      try {
        await atomicUpdatePoolBalance(poolKey, salary.total_amount);
      } catch (compensateErr) {
    ...
    const { data: updatedSalary, error: updateError } = await db.from('salary_payments').update({
    ...
    }).eq('id', id).eq('status', 'approved').select(`
